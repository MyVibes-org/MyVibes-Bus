  ...
  for (const category of CATEGORIES) {
    ...
    try {
      ...
      for (const sh of shapes) {
        ...
        shapesByShapeId
          .get(sh.shape_id)
          ?.push([parseFloat(sh.shape_pt_lat), parseFloat(sh.shape_pt_lon)]);
      }
      ...
      for (const [routeId, rTrips] of routeTrips) {
        ...
        for (const directionId of ["0", "1"] as const) {
          const tripsInDir = rTrips.filter(
          ...
          // Sort by sequence
          selectedStopTimes.sort(
            (a, b) => parseInt(a.stop_sequence) - parseInt(b.stop_sequence)
          );

          const stopsOnRoute: StopOnRoute[] = selectedStopTimes.map(
            (st, index) => {
